...
async function init() {
  ...
  if (profileError || !profile) {
    console.error('Profil introuvable dans civil_profiles pour cet utilisateur :', profileError);
    document.getElementById('user-name').textContent = '⚠ profil introuvable';
    document.getElementById('role-badge').textContent = 'erreur';
    page.hidden = false;
    return;
  }
  ...
  document.getElementById('user-name').textContent = profile.full_name?.split(' ')[0] || '(nom vide)';
  document.getElementById('role-badge').textContent = profile.role || '(rôle vide)';
  ...
  if (profile?.role === 'admin') {
    ...
    // ---------- Utilisateurs : seul canal légitime de promotion admin ----------
...
function renderCounts(profiles, projects, creations, teacherRequests) {
  ...
  const creationsMasquees = creations.filter(c => c.is_hidden).length;
  ...
  document.getElementById('count-creations').textContent = creations.length - creationsMasquees;
  document.getElementById('count-creations-masquees').textContent = creationsMasquees;
...
function renderChart(profiles) {
  ...
  new Chart(document.getElementById('activity-chart'), {
    ...
    data: {
      labels: jours,
      datasets: [{ label: 'Inscriptions par jour', data: counts, backgroundColor: '#e3a72e', borderRadius: 3 }]
    },
    options: {
      plugins: { legend: { display: false } },
      scales: {
        x: { ticks: { color: '#a49c8f', font: { size: 9 } }, grid: { color: 'rgba(255,255,255,0.06)' } },
        y: { ticks: { color: '#a49c8f', precision: 0, stepSize: 1, font: { size: 10 } }, grid: { color: 'rgba(255,255,255,0.06)' }, beginAtZero: true }
      }
    }
  });
}
...
// ============================================================
// UTILISATEURS — admin uniquement. Seul canal légitime pour
// changer le rôle d'un compte, y compris la promotion admin.
// ============================================================
async function loadUsers() {
  ...
  if (error || !users) {
    container.innerHTML = `<p class="empty-note">Impossible de charger la liste.</p>`;
    return;
  }

  if (users.length === 0) {
    container.innerHTML = `<p class="empty-note">Aucun inscrit pour le moment.</p>`;
    return;
  }
  ...
  users.forEach(u => {
    ...
    row.innerHTML = `
      <span class="user-nom">${escapeHtml(u.full_name || 'Sans nom')}</span>
  ...
  container.querySelectorAll('.user-role-select').forEach(select => {
    select.addEventListener('change', async () => {
      ...
      const statusEl = select.parentElement.querySelector('[data-status]');
      statusEl.textContent = '…';
      statusEl.className = 'user-status';

      const { error } = await supabase.from('civil_profiles').update({ role: nouveauRole }).eq('id', id);

      statusEl.textContent = error ? '✕' : '✓';
      statusEl.className = 'user-status ' + (error ? 'err' : 'ok');
    });
  });
}
