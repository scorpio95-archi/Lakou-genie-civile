...
let badgesByUser = {};
let canAccept = false; // auteur du défi OU enseignant/admin
...
const reponseAccepteeSection = document.getElementById('reponse-acceptee-section');
const reponseAccepteeCard = document.getElementById('reponse-acceptee-card');
...
async function loadSujet() {
  ...
  resoluTag.hidden = !sujet.resolu && !sujet.reponse_acceptee_id;
  ...
  // Qui peut accepter une solution : l'auteur du défi, ou enseignant/admin.
  canAccept = (currentUser && currentUser.id === sujet.auteur_id) || currentRole === 'enseignant' || currentRole === 'admin';

  sujetDetail.hidden = false;
  reponsesSection.hidden = false;

  // Corrigé : plus de check 'visitor' fantôme (l'enum réel n'a pas cette valeur).
  if (currentUser) {
...
// ===================== SOLUTIONS (civil_forum_reponses) =====================
...
function reponseCardHtml(r, isAccepted) {
  ...
  const acceptBtn = canAccept
    ? (isAccepted
        ? `<button class="btn-outline btn-unaccept" data-id="${r.id}">Retirer comme solution acceptée</button>`
        : `<button class="btn-primary btn-accept" data-id="${r.id}">✓ Accepter cette solution</button>`)
    : '';
...
function renderReponses(reponses) {
  ...
  const acceptedId = currentSujet?.reponse_acceptee_id;
  const accepted = acceptedId ? reponses.find(r => r.id === acceptedId) : null;

  // ---------- Bandeau "Solution acceptée" en haut de page ----------
  if (accepted) {
    reponseAccepteeSection.hidden = false;
    reponseAccepteeCard.innerHTML = reponseCardHtml(accepted, true);
    wireAcceptButtons(reponseAccepteeCard);
  } else {
    reponseAccepteeSection.hidden = true;
  }

  // ---------- Liste de toutes les solutions (accepted incluse) ----------
  reponses.forEach(r => {
    const card = document.createElement('div');
    card.className = 'reponse-card' + (r.id === acceptedId ? ' reponse-acceptee' : '');
    card.innerHTML = reponseCardHtml(r, r.id === acceptedId);
    reponsesList.appendChild(card);
  });

  wireAcceptButtons(reponsesList);
}

function wireAcceptButtons(container) {
  container.querySelectorAll('.btn-accept').forEach(btn => {
    btn.addEventListener('click', () => setReponseAcceptee(btn.dataset.id));
  });
  container.querySelectorAll('.btn-unaccept').forEach(btn => {
    btn.addEventListener('click', () => setReponseAcceptee(null));
  });
}

async function setReponseAcceptee(reponseId) {
  const { error } = await supabase
    .from('civil_forum_sujets')
    .update({ reponse_acceptee_id: reponseId, resolu: !!reponseId })
    .eq('id', sujetId);
  ...
  currentSujet.reponse_acceptee_id = reponseId;
  currentSujet.resolu = !!reponseId;
  resoluTag.hidden = !reponseId;
  await loadReponses();
}

// ===================== ENVOI D'UNE SOLUTION =====================
reponseForm.addEventListener('submit', async (e) => {
  ...
  reponseSubmitBtn.textContent = 'Proposer cette solution';
