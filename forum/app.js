...
/* ⚠️ ASSOMPTIONS À VÉRIFIER AVEC SÉBASTIEN :
   - civil_profiles.role prend les valeurs 'visitor', 'etudiant', 'enseignant', 'admin'.
   - Table partagée "civil_profiles" (full_name, avatar_url, role) pour les jointures auteur. */
...
function renderFeed() {
  ...
  filtered.forEach(sujet => {
    ...
    card.href = `/forum/sujet/index.html?id=${sujet.id}`;
    card.innerHTML = `
      <div class="sujet-top-row">
        ${cat ? `<span class="sujet-cat-tag">${escapeHtml(cat.nom)}</span>` : ''}
        ${sujet.resolu ? `<span class="sujet-resolu-tag">✓ Résolu</span>` : ''}
      </div>
...
form.addEventListener('submit', async (e) => {
  ...
  submitBtn.textContent = 'Publier le sujet';
  ...
  window.location.href = `/forum/sujet/index.html?id=${data.id}`;
});
