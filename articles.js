...
async function init() {
  ...
  if (session) {
    ...
    if (currentRole) { // corrigé : 'visitor' n'existe pas dans l'enum réel
...
function renderFeed() {
  ...
  allArticles.forEach(article => {
    ...
    const avatarUrl = article.civil_profiles?.avatar_url || 'https://api.dicebear.com/7.x/shapes/svg?seed=' + article.author_id;
     ...
     card.innerHTML = `
      <div class="article-author-row">
        ...
        <div class="author-info">
          <span class="author-name">${escapeHtml(article.civil_profiles?.full_name || 'Utilisateur')}</span>
